...
if($current_numb == count($_SESSION['numbers'])) {
    ...
    $queryAd = $link->query("SELECT * FROM Ad_Id WHERE ad_id='".$_SESSION['numbers'][$current_numb]."'");
    $rowAd = mysqli_fetch_array($queryAd, MYSQLI_ASSOC);
    
    $query = $link->query("SELECT * FROM Tokenbase WHERE access_token='".$rowAd['access_token']."'");
    ...
    if($ERROR == '') {
        ...
        $url = "https://graph.facebook.com/v7.0/".$rowAd['ad_id'];
    ...
    if($ERROR == '') 
        if($output->success) 
            $queryUp = $link->query("UPDATE Ad_Id SET state='ACTIVE' WHERE ad_id='".$rowAd['ad_id']."'");
    else {
