...
if($current_numb == count($_SESSION['info'])) {
    ...
    //if($row['policy'] == '') {
        ...
        if($ERROR == '') {
            ...
            $queryUp = $link->query("UPDATE Tokenbase SET policy = '+' WHERE access_token = '".$access_token."'");
            
            $_SESSION['query'][] = $queryUp;
        
        }
    //}
    
    
    $log = "APPLY POLICY ".date("H:i:m d:m:Y")."\n".json_encode($_SESSION)."\n".date("H:i:m d:m:Y", $row['reftime'])."\n".json_encode($firstoutput)."\n".json_encode($output)."\n\n\n";
    file_put_contents($_SERVER['DOCUMENT_ROOT']."/logs.txt", $log, FILE_APPEND);
    
    header('Location: applyPolicy.php');
}
?>
