...
//$queryUp = $link->query("UPDATE Tokenbase SET state='Selfy' WHERE access_token='EAABsbCS1iHgBANRDFyLVASFveiNz8RxX4EAaXWBdm6Wy2q3ZB9F4qnZBtfBEnKnuDWux0jzanI4DIGJ8TyazGe8FeuIZBGSbGHx0yxxhuilHZClGPemup88B4eZAAhQBs6YeLf7SqHEofVSIw25ZCGZCzuCT7ZCYQ0fzUZCL4oXrJy8ecKzUwug7fOMiRCnzjZASsZD'"); 
//print_r($queryUp);  
...
if($current_numb == count($_SESSION['numbers'])) {
    ...
    $query = $link->query("SELECT access_token FROM Rk_Id");
    $arr = [];
    ...
    if($counts[$rowRk['access_token']] != 1) {
        $_SESSION['current_numb']  = (int)($_SESSION['current_numb']) + $counts[$rowRk['access_token']];
    ...
    if($ERROR == '') {
        ...
        if($output->error) {
            ...
            if($output->error->error_subcode == '459') {
                $queryUp = $link->query("UPDATE Rk_Id SET state='ERROR' WHERE access_token='".$access_token."'");
                $queryUp = $link->query("UPDATE Tokenbase SET state='Selfy' WHERE number='".$access_token."'");
            }
            else if($output->error->error_subcode == '452'){
                $queryUp = $link->query("UPDATE Rk_Id SET state='ERROR' WHERE access_token='".$access_token."'");
                $queryUp = $link->query("UPDATE Tokenbase SET state='Check Passwd' WHERE access_token='".$access_token."'");
                ...
                foreach($data->campaigns->data as $campaing) {
                    ...
                    foreach($campaing->adsets->data as $adsets) {
                        ...
                        foreach($adsets->ads->data as $ad) {
                            ...
                            $queryUp = $link->query("INSERT INTO Ad_Id (link,ctr,cpc,cpm,impressions,spend,clicks,creative,name,cpl,results,state,ad_id,access_token,adset_id) VALUES ('".$site."',".$ctr."','".$cpc."','".$cpm."','".$impressions."', '".$spent."', '".$clicks."', '".$creative."', '".$name."', '".$cpl."', '".$result."', '".$effective_status."', '".$ad_id."', '".$access_token."', '".$adset_id."')");
                            if(!$queryUp) 
    ...
    header('Location: refreshCabs.php');
}
?>