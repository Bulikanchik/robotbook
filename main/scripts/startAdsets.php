...
if($current_numb == count($_SESSION['numbers'])) {
    ...
    if($ERROR == '') {
        ...
        $url = "https://graph.facebook.com/v7.0/".$rowAdset['adset_id'];
    ...
    if($ERROR == '') 
        if($output->success) 
            $queryUp = $link->query("UPDATE Adset_Id SET state='ACTIVE' WHERE adset_id='".$rowAdset['adset_id']."'");
    else {
    ...
    header('Location: startAdsets.php');
}
