...
if($current_numb == count($_SESSION['numbers'])) {
    ...
    $query = $link->query("SELECT access_token FROM Adset_Id");
    $arr = [];
    ...
    $queryAdset = $link->query("SELECT * FROM Adset_Id WHERE adset_id='".$_SESSION['numbers'][$current_numb]."'");
    $rowAdset = mysqli_fetch_array($queryAdset, MYSQLI_ASSOC);
    
    $counts = array_count_values($arr);
    if($counts[$rowAdset['access_token']] != 1) {
        $_SESSION['current_numb']  = (int)($_SESSION['current_numb']) + $counts[$rowAdset['access_token']];
    ...
    $query = $link->query("SELECT * FROM Tokenbase WHERE access_token='".$rowAdset['access_token']."'");
        ...
        foreach($output->data as $data) {
            ...
            foreach($data->campaigns->data as $campaing) {
                ...
                foreach($campaing->adsets->data as $adsets) {
                    ...
                    if(in_array($adset_id, $_SESSION['numbers']))    
                        if(!$queryUp)      
    ...
    header('Location: refreshAdsets.php');
}
