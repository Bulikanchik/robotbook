...
if($current_numb == count($_SESSION['numbers'])) {
    ...
    if($rowRk['pixel_id'] == '') {
        ...
        if($ERROR == '') {
            ...
            $url =  "https://graph.facebook.com/v7.0/".$rowRk['rk_id']."/adspixels";
         
         
            $post_data = array (
                "access_token" => $row['access_token'],
                "name" => 'Пиксель аккаунта «'.$row['name'].'»'
            );
    ...
    header('Location: createPixel.php');
}
